package cn.cjp.demo.hibernate.shards.xml;
...
public class Main {
	...
	private static void ShardsTestCreate(SessionFactory factory) {
		...
		try {
			...
			session.save(new ContactEntity("a@cnblogs.com", "123",
					"AAA", "a@cnblogs.com"));
			session.save(new ContactEntity("b@cnblogs.com", "123",
					"BBB", "b@cnblogs.com"));
			session.save(new ContactEntity("c@cnblogs.com", "123",
					"CCC", "c@cnblogs.com"));
			transaction.commit();
		} catch (Exception e) {
			if (transaction != null)
